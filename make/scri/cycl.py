...
def main(owne, Math):
	...
	acti = owne["acti"]
	cycl = owne[acti]
	leftArm_ = owne["leftArm_"]
	righArm_ = owne["righArm_"]
	leftLeg_ = owne["leftLeg_"]
	righLeg_ = owne["righLeg_"]
	if cycl == 1:
		owne["cyclTime"] = 0.0
		owne[acti] = 2
		cycl = 2
	if cycl == 2:
		...
		if cyclArms == True:
			...
			if leftArm_: Cycl(trac = owne.name + ".axle.arms.l", t = t, acti = acti, radi = cyclRadi, spee = cyclSpee, uppe = owne.name + ".shou.l", lowe = owne.name + ".elbo.l", end = owne.name + ".hand.l", righ = False, arms = True, armsRati = armsRati, uppeLeng = uppeLeng, loweLeng = loweLeng, faci = faci, owne = owne, Math = Math)
			if righArm_: Cycl(trac = owne.name + ".axle.arms.r", t = t, acti = acti, radi = cyclRadi, spee = cyclSpee, uppe = owne.name + ".shou.r", lowe = owne.name + ".elbo.r", end = owne.name + ".hand.r", righ = True,  arms = True, armsRati = armsRati, uppeLeng = uppeLeng, loweLeng = loweLeng, faci = faci, owne = owne, Math = Math)
		osci = owne[acti + "Osci"]
		if osci != 0.0:
